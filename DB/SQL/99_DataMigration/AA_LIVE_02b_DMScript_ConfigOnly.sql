...
-- CONVERT(date, [date of birth], 103), CONCAT(english, mandarin, malay, tamil, others), gender,
--'Blk: , Street: ' + address + ', Unit: ', [postal code], race, citizenship,

INSERT INTO [phs].dbo.PreRegistration ([Nric],[FullName],[Salutation],[HomeNumber],[MobileNumber],[DateOfBirth],[Language],[Gender],[Address],[PostalCode],[Race],[Citizenship],
[OPTION1],[OPTION2],[OPTION3],[OPTION4],[OPTION5],[OPTION6],[OPTION7],[OPTION8],[OPTION9],[OPTION10],[CreatedDateTime], EntryId, [PreferedTime]) 
select NRIC, [Name as in NRIC], '', isnull([Home number], ''), ISNULL([Mobile number], ''),  CONVERT(date, [Date of birth (dd/mm/yy)], 103), isnull([Preferred language], ''), ISNULL(Sex,''),
CONCAT('Blk: ', [Block/House No#], ', Street: ', [Street Address], ', Unit: ', Unit), [Zip Code], ISNULL(Race, ''), ISNULL(Citizenship, ''), 
isnull(Email, ''), isnull([Knows Criteria for Phlebotomy], ''), '','','','','','','','',GETDATE(), cast((12345) as varbinary(32)) as uniqueidentifier, '' 
 from prereg
